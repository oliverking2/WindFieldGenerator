...
def updateGraph(windVelocityArray):
    ...
    """
    elif windDirectionDeg > 0:
        global count
        count += 1
        oldWindVelocityArray = windVelocityArray[1:]  # removes the last row of the matrix
        newLine = np.random.randint(minWindVelocity, maxWindVelocity, velocitySize + 1)
        windVelocityArrayPreShift = np.vstack((oldWindVelocityArray, newLine))
        if count == 4:
            windVelocityArray = []
            for row in windVelocityArrayPreShift:
                windVelocityArray.append(np.append(row[1:], np.random.randint(minWindVelocity, maxWindVelocity)))
            count = 0
        else:
            windVelocityArray = windVelocityArrayPreShift
